...
class GenerateHighVolumeSimulationUseCaseTest {
    ...
    @Test
    fun `simulation rotates eligible parts for broader data coverage`() {
        val inspectionRepository = RecordingInspectionRepository()
        val useCase =
            GenerateHighVolumeSimulationUseCase(
                inspectionRepository = inspectionRepository,
        ...
        useCase.execute(days = 2, density = 1, seed = 7L)

        val insertedPartIds = inspectionRepository.insertedInputs.map { it.partId }.toSet()
        assertEquals(setOf(11L, 12L, 13L), insertedPartIds)
    }
}
